...
#include "Editor/AttributeEditor.as"
...
{
    String[] resources = { "Editor/EditorInspector_Attribute.xml", "Editor/EditorInspector_Variable.xml",
                            "Editor/EditorInspector_Style.xml", "Editor/EditorInspector_Tags.xml" };
...
void CreateAttributeInspectorWindow()
{
    ...
    attributeInspectorWindow = LoadEditorUI("Editor/EditorInspectorWindow.xml");
...
void ResizeAttributeInspectorWindow()
{
    attributeInspectorWindow.SetSize(360, ui.root.height - 68);
    attributeInspectorWindow.SetPosition(ui.root.width - 360, 0);
...
void SetAttributeEditorID(UIElement@ attrEdit, Array<Serializable@>@ serializables)
{
    ...
    default:
        ...
        attrEdit.vars[SERIALIZABLE_PTRS_VAR] = ids;
        break;
    }
}
...
Array<Serializable@>@ GetAttributeEditorTargets(UIElement@ attrEdit)
{
    ...
    else
    {
		...
		{
            variant = attrEdit.GetVar(SERIALIZABLE_PTRS_VAR);
            if (!variant.empty)
            {
                Array<Variant>@ ids = variant.GetVariantVector();
                for (uint i = 0; i < ids.length; ++i)
                {
                    Serializable@ serializable = cast<Serializable>(ids[i].GetPtr());
                    if (serializable !is null)
                        ret.Push(serializable);
                }
            }
		}
    }
