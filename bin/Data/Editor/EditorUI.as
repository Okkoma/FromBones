...
//    ui.root.RemoveAllChildren();
    ...
    uiStyle = GetEditorUIXMLFile("Editor/DefaultStyle.xml");
//    ui.root.defaultStyle = uiStyle;
    iconStyle = GetEditorUIXMLFile("Editor/EditorIcons.xml");

    editorUI = ui.root.CreateChild("UIElement", "EditorUI");
    editorUI.SetFixedSize(ui.root.width, ui.root.height);
...
void RemoveUI()
{
    editorUI.RemoveChild(toolBar);
    editorUI.RemoveChild(hierarchyWindow);
    editorUI.RemoveChild(attributeInspectorWindow);
    ui.root.RemoveChild(editorUI);
    editorUI = null;
    toolBar = null;
    hierarchyWindow = null;
    attributeInspectorWindow = null;
}
...
//    uiMenuBar.SetFixedWidth(graphics.width);
    ...
    // Resize tool bar
    toolBar.SetFixedWidth(ui.root.width);
    toolBar.SetPosition(0, ui.root.height - 68);

    // Resize secondary tool bar
//    secondaryToolBar.SetFixedHeight(graphics.height);

    // Relayout windows
//    Array<UIElement@> children = editorUI.GetChildren();
...
void CreateQuickMenu()
{
    ...
    quickMenu = LoadEditorUI("Editor/EditorQuickMenu.xml");
...
{
    contextMenu = LoadEditorUI("Editor/EditorContextMenu.xml");
}
...
void HandleHotKeysStandard(VariantMap& eventData)
{
    ...
    log.Info("HandleHotKeysStandard key=" + String(key));
...
{
    HideUI(!uiHidden);
    //log.Info("ToggleUI visible=" + String(!uiHidden));
...
void HideUI(bool hide = true)
{
    ...
    bool visible = !uiHidden;
    Array<UIElement@> children = editorUI.GetChildren();
    log.Info("hideUI visible=" + String(hide) + "numChildren=" + String(children.length));
    for (uint i = 0; i < children.length; ++i)
    {
        // Cursor is excluded
        if (children[i].type == CURSOR_TYPE)
            continue;

        //log.Info("hideUI visible=" + String(!hide) + " " + children[i].name + " entryVisible=" + String(children[i].visible));
        ...
        //log.Info("... visible=" + String(children[i].visible));
    }
}
...
{
    UIElement@ element = ui.LoadLayout(GetEditorUIXMLFile(fileName));
    editorUI.AddChild(element);
    return element;
}
