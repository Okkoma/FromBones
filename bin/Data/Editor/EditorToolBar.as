...
void CreateToolBar()
{
    ...
    toolBar.SetFixedSize(ui.root.width, 68);
    toolBar.SetPosition(0, ui.root.height - 68);

    editorUI.AddChild(toolBar);
...
Button@ CreateToolBarButton(const String&in title)
{
    ...
    CreateToolTip(button, title, IntVector2(button.width/2, -20));
...
CheckBox@ CreateToolBarToggle(const String&in title)
{
    ...
    CreateToolTip(toggle, title, IntVector2(toggle.width/2, -20));
...
UIElement@ CreateToolTip(UIElement@ parent, const String&in title, const IntVector2&in offset)
{
    ...
    textHolder.horizontalAlignment = HA_CENTER;
...
void ToolBarSpawnActor(StringHash eventType, VariantMap& eventData)
{
    ...
    if (edit.checked)
        spawnCategory = SPAWN_ACTOR;
    else if (spawnCategory == SPAWN_ACTOR)
        spawnCategory = SPAWN_NONE;
    toolBarDirty = true;
}
...
void UpdateDirtyToolBar()
{
    ...
    CheckBox@ spawnActorToogle = toolBar.GetChild("Actor", true);
    spawnActorToogle.checked = (spawnCategory == SPAWN_ACTOR);

    CheckBox@ spawnMonsterToogle = toolBar.GetChild("Monster", true);
    spawnMonsterToogle.checked = (spawnCategory == SPAWN_MONSTER);

    CheckBox@ spawnPlantToogle = toolBar.GetChild("Plant", true);
    spawnPlantToogle.checked = (spawnCategory == SPAWN_PLANT);
...
//        fillSolidToggle.checked = fillMode == FILL_SOLID;
    ...
    {
        SubscribeToEvent(toolBar.GetChild("Actor", true), "Toggled", "ToolBarSpawnActor");
        SubscribeToEvent(toolBar.GetChild("Monster", true), "Toggled", "ToolBarSpawnMonster");
        SubscribeToEvent(toolBar.GetChild("Plant", true), "Toggled", "ToolBarSpawnPlant");
