...
const StringHash SPLINEPATH2D_TYPE("SplinePath2D");
...
void CreateHierarchyWindow()
{
    ...
    hierarchyWindow = LoadEditorUI("Editor/EditorHierarchyWindow.xml");
...
void ResizeHierarchyWindow()
{
    hierarchyWindow.SetSize(360, ui.root.height - 68);
    hierarchyWindow.SetPosition(ui.root.width-720, 0);
...
//            log.Info("UpdateHierarchy ... node=" + node.name);

            text.text = GetNodeTitle(node);
            text.color = nodeTextColor;
            SetIconEnabledColor(text, node.enabled);

            if (node.name == "PreLoad" || node.name == "PreLoadGOT" || node.name == "ObjectPool" || node.name == "Pool" || node.name == "PrepareNodes")
            {
//                log.Info("UpdateHierarchy OK node=" + node.name);
                break;
            }
...
void HandleHierarchyListSelectionChange()
{
    ...
    if (hierarchyList.selections.length > 0)
    {
        // Enable Expand/Collapse button when there is selection
//        EnableExpandCollapseButtons(indices.length > 0);

        for (uint i = 0; i < hierarchyList.selections.length; ++i)
        {
            uint index = hierarchyList.selections[i];
            UIElement@ item = hierarchyList.items[index];
        ...
        log.Info("selectedNodes="+String(selectedNodes.length));
        ...
        log.Info("node edited =" + editNodes[0].name);
    }
