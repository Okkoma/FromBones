...
String GetComponentTitle(Component@ component)
{
    ...
    if (showID)
    {
        if (component.replicated)
            ret += " (" + String(component.id) + ")";
        else
            ret += " (Local " + String(component.id) + ")";
...
void HandleHierarchyListSelectionChange()
{
    ...
    scene2DAnimator.UpdateSelection();
}
...
void HandleHierarchyItemClick(StringHash eventType, VariantMap& eventData)
{
    ...
    else if (type == ITEM_NODE)
    {
        ...
        if (cast<Scene>(editNode) !is null)
          actions.Push(CreateContextMenuItem("Reset time", "HandleHierarchyContextResetTime"));
        actions.Push(CreateContextMenuItem("Enable/disable", "HandleHierarchyContextEnableDisable"));
    ...
    else if (itemType == ITEM_COMPONENT)
    {
        ...
        else
        {
            if (targetComponent !is null && sourceNodes.length > 0)
            {
                ...
                // Drag node to SplinePath to make it a control point
                SplinePath2D@ spline2d = cast<SplinePath2D>(targetComponent);
                if (spline2d !is null)
                {
                    // Save undo action
                    EditAttributeAction action;
                    uint attrIndex = GetAttributeIndex(spline2d, "Point Nodes");
                    Variant oldIDs = spline2d.attributes[attrIndex];

                    for (uint i = 0; i < sourceNodes.length; ++i)
                        spline2d.AddPointNode(sourceNodes[i]);

                    action.Define(spline2d, attrIndex, oldIDs);
                    SaveEditAction(action);
                    SetSceneModified();
                    UpdateAttributeInspector(false);
                }
    ...
    else if (targetItemType == ITEM_COMPONENT)
    {
        ...
        else
        {
            ...
            if (sourceNode !is null && targetComponent !is null && (targetComponent.type == STATICMODELGROUP_TYPE ||
                targetComponent.type == CONSTRAINT_TYPE || targetComponent.type == SPLINEPATH_TYPE || targetComponent.type == SPLINEPATH2D_TYPE))
                return true;
...
void HandleHierarchyContextResetTime()
{
    SceneResetTime();
}
