...
bool SaveScene(const String&in fileName)
{
    ...
    // Reset all SceneAnimation2D before saving
    scene2DAnimator.ResetTime();
...
void StopSceneUpdate()
{
    ...
    get_log().Info("StopSceneUpdate ... OK !");
}
...
bool SceneResetTime()
{
    if (editNode !is null)
    {
        Scene@ scene = cast<Scene>(editNode);
        scene.set_elapsedTime(0.0);

        // Reset SplinePaths
        Array<Component@>@ components = scene.GetComponents("SplinePath", true);
        for (uint i = 0; i < components.length; ++i)
        {
            SplinePath@ splinepath = cast<SplinePath>(components[i]);
            splinepath.Reset();
            splinepath.Move(0.0);
        }
