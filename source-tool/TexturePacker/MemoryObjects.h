...
public:
    ...
    Matrix2D(unsigned width, unsigned height) { Resize(width, height); }
    ...
    void SetBufferFrom(const T* buffer) { memcpy(Buffer(), buffer, sizeof(T) * Size()); }
    void SetBufferValue(unsigned value) { memset(Buffer(), value, sizeof(T) * Size()); }
