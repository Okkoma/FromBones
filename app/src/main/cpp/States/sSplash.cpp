...
void SplashState::ResizeScreen()
{
    ...
    {
        texture = static_cast<Texture2D*>(sprite->GetTexture());
        IntVector2 size(width*0.6f, height*0.6f);
        GameHelpers::ApplySizeRatio(texture->GetWidth(), texture->GetHeight(), size);
        sprite->SetSize(size);
        sprite->SetHotSpot(size.x_ / 2.f, size.y_ / 2.f);
        sprite->SetPosition(width / 2.f, height / 2.f);
    ...
    {
        texture = static_cast<Texture2D*>(sprite->GetTexture());
        IntVector2 size(width*0.1f, height*0.1f);
        GameHelpers::ApplySizeRatio(texture->GetWidth(), texture->GetHeight(), size);
        sprite->SetSize(size);
        sprite->SetHotSpot(size.x_ / 2.f, size.y_ / 2.f);
...
void SplashState::Create()
{
    ...
    SharedPtr<Texture2D> backtexture   = cache->GetTempResource<Texture2D>("Textures/UI/splashback.png");
    SharedPtr<Texture2D> splashtexture = cache->GetTempResource<Texture2D>("Textures/UI/splasheffect.png");
    SharedPtr<Texture2D> okkotexture   = cache->GetTempResource<Texture2D>("Textures/UI/okkologo.png");
    SharedPtr<Texture2D> urhotexture   = cache->GetTempResource<Texture2D>("Textures/UI/urho3dlogo.png");

    for (unsigned i=0; i < sprites_.Size(); ++i)
        GameContext::Get().ui_->GetRoot()->RemoveChild(sprites_[i]);
    sprites_.Clear();
    ...
    if (backtexture)
    {
        ...
        sprite = GameContext::Get().ui_->GetRoot()->CreateChild<Sprite>();
        sprites_.Push(sprite);
        size = IntVector2(width, height);
        GameHelpers::ApplySizeRatio(backtexture->GetWidth(), backtexture->GetHeight(), size);
        ...
        sprite->SetObjectAnimation(effectAnimation);
    }
    ...
    if (splashtexture)
    {
        ...
        sprites_.Push(sprite);
        size = IntVector2(width, height);
        GameHelpers::ApplySizeRatio(splashtexture->GetWidth(), splashtexture->GetHeight(), size);
    ...
    if (okkotexture)
    {
        sprite = GameContext::Get().ui_->GetRoot()->CreateChild<Sprite>();
        sprites_.Push(sprite);
        size = IntVector2(width*0.6f, height*0.6f);
        GameHelpers::ApplySizeRatio(okkotexture->GetWidth(), okkotexture->GetHeight(), size);
    ...
    if (urhotexture)
    {
        sprite = GameContext::Get().ui_->GetRoot()->CreateChild<Sprite>();
        sprites_.Push(sprite);
        size = IntVector2(width*0.1f, height*0.1f);
        GameHelpers::ApplySizeRatio(urhotexture->GetWidth(), urhotexture->GetHeight(), size);
...
void SplashState::Begin()
{
    ...
    DelayInformer* delay = new DelayInformer(this, 1.05f*DURATION, SPLASH_STOP);
