...
bool wrenImportModuleWithName(WrenVM* vm, const char* modulename)
{
    Value module = wrenStringFormat(vm, "$", modulename);
    wrenPushRoot(vm, AS_OBJ(module));

    Value value = wrenImportModule(vm, module);
...
void wrenRunHandle(WrenVM* vm, WrenHandle* handle)
{
    ObjClosure* closure = AS_CLOSURE(handle->value);
    wrenPushRoot(vm, (Obj*)closure);
    ObjFiber* fiber = wrenNewFiber(vm, closure);
    wrenPopRoot(vm);
...
WrenHandle* wrenCompileToHandle(WrenVM* vm, const char* source)
{
    ObjClosure* closure = wrenCompileSource(vm, "main", source, false, true);
    if (closure == NULL) return NULL;
    //WrenHandle* handle = wrenMakeHandle(vm, OBJ_VAL(closure->fn));
    //handle->value = OBJ_VAL(wrenNewClosure(vm, closure->fn));

    return wrenMakeHandle(vm, OBJ_VAL(closure->fn));
}

WrenInterpretResult wrenCompileTestSource(WrenVM* vm, const char* source)
{
    ObjClosure* closure = wrenCompileSource(vm, "main", source, false, true);
    if (closure == NULL) return WREN_RESULT_COMPILE_ERROR;
    ...
    return handle ? WREN_RESULT_SUCCESS : WREN_RESULT_RUNTIME_ERROR;
}
