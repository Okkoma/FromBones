...
void drawSpanRGBA(SSpan span, int y, CArray2Drgbav1 *img, CRGBAModuleBase *m)
{
    float xdiff=span.x2_-span.x1_;
    if(xdiff==0) return;
...
void drawSpansBetweenEdgesRGBA(SEdge e1, SEdge e2, CArray2Drgbav1 *img, CRGBAModuleBase *m)
{
    float e1ydiff=(e1.y2_-e1.y1_);
    if(e1ydiff==0) return;
...
void rasterizeRGBATriangle(int u1, int v1, CVec3f p1, int u2, int v2, CVec3f p2, int u3, int v3, CVec3f p3, CArray2Drgbav1 *a, CRGBAModuleBase *f)
{
    SEdge edges[3]=
    {
        SEdge(u1,v1,p1, u2,v2,p2),
