...
float CImplicitBasisFunction::get(float x, float y)
{
    float nx,ny;
    nx = x*cos2d-y*sin2d;
    ny = y*cos2d+x*sin2d;
...
float CImplicitBasisFunction::get(float x, float y, float z)
{
    float nx, ny, nz;
...
float CImplicitBasisFunction::get(float x, float y, float z, float w)
{
    float nx, ny, nz;
    nx = (m_rotmatrix[0][0]*x) + (m_rotmatrix[1][0]*y) + (m_rotmatrix[2][0]*z);
    ny = (m_rotmatrix[0][1]*x) + (m_rotmatrix[1][1]*y) + (m_rotmatrix[2][1]*z);
...
float CImplicitBasisFunction::get(float x, float y, float z, float w, float u, float v)
{
    float nx, ny, nz;
    nx = (m_rotmatrix[0][0]*x) + (m_rotmatrix[1][0]*y) + (m_rotmatrix[2][0]*z);
    ny = (m_rotmatrix[0][1]*x) + (m_rotmatrix[1][1]*y) + (m_rotmatrix[2][1]*z);
...
void CImplicitBasisFunction::setRotationAngle(float x, float y, float z, float angle)
{
