...
private:
    ...
    T calcRasterValue(T vsrc, T vdest, float blend, int mode)
    {
        ...
        case RTM_Add:
            return vsrc+vdest;
            break;
        case RTM_SubtractSrc:
            return (vdest-vsrc);
            break;
        case RTM_SubtractDest:
            return (vsrc-vdest);
            break;
        case RTM_Average:
            return (T)((vsrc+vdest)*0.5f);
            break;
        ...
        case RTM_Multiply:
            ...
            break;
        case RTM_Blend:
            return vdest + (T)((vsrc-vdest)*blend);
        default:
            return vsrc;
            break;
        }
...
public:
    ...
    T sampleRow(TArray2D<T> *src, TArray2D<float> *blur, float x, float y)
    {
    ...
    T sampleCol(TArray2D<T> *src, TArray2D<float> *blur, float x, float y)
    {
        // Apply sample kernel to the rows of the source image to sample down
        int bw=blur->width();
        T total=T(0);
    ...
    void buildSampleKernel(TArray2D<float> *kernel, int size)
    {
        kernel->resize(size,1);
        float c=(float)size / 2.0;
        float total=0;
        for(int x=0; x<size; ++x)
        {
            float d=std::abs((float)x-c) / c;
            d=1.0-d;
            kernel->set(x,0,d);
            total+=d;
        }
    ...
    void applyBlurRow(TArray2D<T> *src, TArray2D<T> *dest, TArray2D<float> *blur, int x, int y, bool seamless)
    {
        int bw=blur->width();
        T total=T(0);
        float totalweight=0;
    ...
    void applyBlurCol(TArray2D<T> *src, TArray2D<T> *dest, TArray2D<float> *blur, int x, int y, bool seamless)
    {
        int bw=blur->width();
        T total=T(0);
        float totalweight=0;
    ...
    void blur(float blur_size, bool seamless)
    {
        ...
        TArray2D<float> kernelx, kernely;
...
public:
    ...
    TArray3D(size_t width, size_t height, size_t depth)
    {
