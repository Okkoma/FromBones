...
public:
    ...
    const CInstructionIndex& pi() const;
    const CInstructionIndex& e() const;
    const CInstructionIndex& one() const;
    const CInstructionIndex& zero() const;
    const CInstructionIndex& point5() const;
    const CInstructionIndex& sqrt2() const;

    CInstructionIndex constant(float val);
    ...
    CInstructionIndex scaleOffset(CInstructionIndex src, float scale, float offset);

    CInstructionIndex simpleFractalLayer(unsigned int basistype, CInstructionIndex interptypeindex, float layerscale, float layerfreq, unsigned int seed, bool rot=true,
                                         float angle=0.5f, float ax=0.f, float ay=0.f, float az=1.f);
    CInstructionIndex simpleRidgedLayer(unsigned int basistype, CInstructionIndex interptypeindex, float layerscale, float layerfreq, unsigned int seed, bool rot=true,
                                        float angle=0.5f, float ax=0.f, float ay=0.f, float az=1.f);
    CInstructionIndex simpleBillowLayer(unsigned int basistype, CInstructionIndex interptypeindex, float layerscale, float layerfreq, unsigned int seed, bool rot=true,
                                        float angle=0.5f, float ax=0.f, float ay=0.f, float az=1.f);

    CInstructionIndex simplefBm(unsigned int basistype, unsigned int interptype, unsigned int numoctaves, float frequency, unsigned int seed, bool rot=true);
    CInstructionIndex simpleRidgedMultifractal(unsigned int basistype, unsigned int interptype, unsigned int numoctaves, float frequency, unsigned int seed, bool rot=true);
    CInstructionIndex simpleBillow(unsigned int basistype, unsigned int interptype, unsigned int numoctaves, float frequency, unsigned int seed, bool rot=true);
    ...
    CInstructionIndex pushInstruction(unsigned int opcode, int numsrc=0, unsigned int* src=0);

    void setVar(const String& name, float val);
    CInstructionIndex getVar(const String& name);

    const Vector<unsigned >& getCachedIndexes() const
    {
        return cachedindexes_;
    }

    CKernel &operator =(const CKernel &rhs)
    {
        pi_ = rhs.pi_;
        e_ = rhs.e_;
        one_ = rhs.one_;
...
private:
    ...
    Vector<unsigned > cachedindexes_;
    HashMap<String, CInstructionIndex > vars_;
};
