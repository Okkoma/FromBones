...
CKernel::CKernel() :
    pi_(constant(3.14159265358979323846f)),
    e_(constant(2.71828182845904523536f)),
    one_(constant(1.f)),
    zero_(constant(0.f)),
    point5_(constant(0.5f)),
    sqrt2_(constant(sqrt(2.f)))
{ }

CKernel::CKernel(const CKernel &rhs) :
    kernel_(rhs.kernel_),
    cachedindexes_(rhs.cachedindexes_),
    pi_(rhs.pi_),
    e_(rhs.e_),
    one_(rhs.one_),
    zero_(rhs.zero_),
...
const CInstructionIndex& CKernel::pi() const
{
    return pi_;
}

const CInstructionIndex& CKernel::e() const
{
    return e_;
}

const CInstructionIndex& CKernel::one() const
{
    return one_;
}

const CInstructionIndex& CKernel::zero() const
{
    return zero_;
}

const CInstructionIndex& CKernel::point5() const
{
    return point5_;
}

const CInstructionIndex& CKernel::sqrt2() const
{
    return sqrt2_;
}


CInstructionIndex CKernel::constant(float val)
{
    anl::SInstruction i;
    i.outfloat_=val;
...
CInstructionIndex CKernel::simpleFractalLayer(unsigned int basistype, CInstructionIndex interpindex, float layerscale, float layerfreq, unsigned int s, bool rot,
...
CInstructionIndex CKernel::simpleRidgedLayer(unsigned int basistype, CInstructionIndex interpindex, float layerscale, float layerfreq, unsigned int s, bool rot,
        float angle, float ax, float ay, float az)
{
    CInstructionIndex myseed=seed(s);
    CInstructionIndex base=nextIndex();
...
CInstructionIndex CKernel::simpleBillowLayer(unsigned int basistype, CInstructionIndex interpindex, float layerscale, float layerfreq, unsigned int s, bool rot,
        float angle, float ax, float ay, float az)
{
...
//    URHO3D_LOGINFOF("billowlayer scale=%f freq=%f", layerscale, layerfreq);
    ...
    base=abs(base);
    base=multiply(base,constant(2.f));
...
CInstructionIndex CKernel::simpleRidgedMultifractal(unsigned int basistype, unsigned int interptype, unsigned int numoctaves, float frequency, unsigned int seed, bool rot)
{
    ...
    KISS rnd;
    rnd.setSeed(seed);
    float angle = rnd.get01();
    float ax = rnd.get01();
    float ay = rnd.get01();
    float az = rnd.get01();

    CInstructionIndex lastlayer = simpleRidgedLayer(basistype, interpindex, 1.f, 1.f*frequency, seed+10, rot, angle*3.14159265f, ax, ay, az);
    ...
    for(unsigned int c=0; c<numoctaves-1; ++c)
    {
        ...
        ay = rnd.get01();
        az = rnd.get01();
        CInstructionIndex nextlayer=simpleRidgedLayer(basistype, interpindex, 1.f/std::pow(2.f, (float)(c)), std::pow(2.f, (float)(c))*frequency, seed+10+c*1000, rot, angle*3.14159265f, ax, ay, az);
...
CInstructionIndex CKernel::simplefBm(unsigned int basistype, unsigned int interptype, unsigned int numoctaves, float frequency, unsigned int seed, bool rot)
{
    if(numoctaves<1) return 0;
    ...
    CInstructionIndex lastlayer = simpleFractalLayer(basistype, interpindex, 1.f, 1.f*frequency, seed+10, rot, angle*3.14159265f, ax, ay, az);
    ...
    for(unsigned int c=0; c<numoctaves-1; ++c)
    {
        ...
        ay = rnd.get01();
        az = rnd.get01();
        CInstructionIndex nextlayer=simpleFractalLayer(basistype, interpindex, 1.f/std::pow(2.f, (float)(c)), std::pow(2.f, (float)(c))*frequency, seed+10+c*1000, rot, angle*3.14159265f, ax, ay, az);
        lastlayer=add(lastlayer,nextlayer);
    }
    return lastIndex();
}

CInstructionIndex CKernel::simpleBillow(unsigned int basistype, unsigned int interptype, unsigned int numoctaves, float frequency, unsigned int seed, bool rot)
{
    if(numoctaves<1) return 0;
    ...
    CInstructionIndex lastlayer = simpleBillowLayer(basistype, interpindex, 1.f, 1.f*frequency, seed+10, rot, angle*3.14159265f, ax, ay, az);
    ...
    for(unsigned int c=0; c<numoctaves-1; ++c)
    {
        ...
        ay = rnd.get01();
        az = rnd.get01();
        CInstructionIndex nextlayer=simpleBillowLayer(basistype, interpindex, 1.f/std::pow(2.f, (float)(c)), std::pow(2.f, (float)(c))*frequency, seed+10+c*1000, rot, angle*3.14159265f, ax, ay, az);
        lastlayer=add(lastlayer,nextlayer);
    }
    return lastIndex();
}
...
CInstructionIndex CKernel::scaleOffset(CInstructionIndex src, float scale, float offset)
{
...
CInstructionIndex CKernel::pushInstruction(unsigned int opcode, int numsrc, unsigned int* src)
{
    anl::SInstruction i;
    i.opcode_=opcode;
    ...
    if (opcode == OP_CacheArray)
        cachedindexes_.Push(kernel_.Size()-1);

    return lastIndex();
}

void CKernel::setVar(const String& name, float val)
{
    HashMap<String, CInstructionIndex >::Iterator it = vars_.Find(name);
    if (it == vars_.End())
    {
        vars_[name] = constant(val);
        return;
    }

    unsigned s = it->second_.index_;
    if (s < kernel_.Size())
        kernel_[s].outfloat_ = val;
}

CInstructionIndex CKernel::getVar(const String& name)
{
    HashMap<String, CInstructionIndex >::ConstIterator it = vars_.Find(name);

    if (it == vars_.End())
        return zero();

    return it->second_.index_ < kernel_.Size() ? it->second_ : zero();
}
