...
public :
    ...
    ObjectPoolCategory& operator = (const ObjectPoolCategory& obj)
    {
        return *this;
    }
    ...
    bool IsSet() const
    {
        return GOT_ && nodes_.Size() != 0;
    }
    bool IsNodeInPool(Node* node) const;

    unsigned GetNextReplicatedNodeID() const;
    unsigned GetFirstNodeID(CreateMode mode) const
    {
        return mode == REPLICATED && replicatedState_ ? firstReplicatedNodeID_ : firstNodeID_;
    }
    unsigned GetLastNodeID(CreateMode mode) const
    {
        return mode == REPLICATED && replicatedState_ ? lastReplicatedNodeID_ :  lastNodeID_;
    }
    unsigned GetFirstComponentID(CreateMode mode) const
    {
        return mode == REPLICATED && replicatedState_ ? firstReplicatedComponentID_ : firstComponentID_;
    }
    unsigned GetLastComponentID(CreateMode mode) const
    {
        return mode == REPLICATED && replicatedState_ ? lastReplicatedComponentID_ :  lastComponentID_;
    }
    ...
    void ChangeToID(Node* node, unsigned newid);
...
//    void ResetAttributes(Node* node);
...
public :
    ...
    bool AreCategoriesUpdated() const
    {
        return categoriesToUpdate_.Size() == 0;
    }

    void RestoreCategories(bool allObjects=false);
    void Stop();

    const HashMap<StringHash, ObjectPoolCategory >& GetCategories() const
    {
        return categories_;
    }
    void DumpCategories() const;

    static void SetForceLocalMode(bool enable)
    {
        forceLocalMode_ = enable;
    }
    static ObjectPool* Get()
    {
        return pool_;
    }
    static const String& GetDebugData()
    {
        UpdateDebugData();
        return debugTxt_;
    }
    ...
    static bool IsInPool(Node* node);
