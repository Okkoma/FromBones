    ...
    networkMode_(AUTOMODE)
{
    ...
    net_->RegisterRemoteEvent(NET_SERVERSEEDTIME);
...
void GameNetwork::PushObjectCommand(NetCommand pcmd, VariantMap* eventDataPtr)
{
...
//    URHO3D_LOGERRORF("GameNetwork() - PushObjectCommand : clientid=%d(%d) stamp=%u(%u) cmd=%s !",
//                     cmd.clientId_, clientID_, cmd.stamp_, clientSideConnection_ ? clientSideConnection_->GetClientObjCmd() : 0, netCommandNames[pcmd]);
}
...
void GameNetwork::ChangeEquipment(VariantMap& eventData)
{
    ...
    if (type)
    {
        ...
        if (controller)
            controller->ChangeAvatar(type, entityid);
    }

    // Change Equipment
    if (serverMode_)
    {
        GOC_Inventory::NetServerSetSlot(node, eventData);
    }
    else
    {
...
void GameNetwork::LoadEquipment(VariantMap& eventData)
{
    Node* node = GameContext::Get().rootScene_->GetNode(eventData[Net_ObjectCommand::P_NODEID].GetUInt());
    if (!node)
    {
        URHO3D_LOGERRORF("GameNetwork() - LoadEquipment ... No Node !");
        return;
    }
    ...
    if (!controller)
    {
        URHO3D_LOGERRORF("GameNetwork() - LoadEquipment ... No Controller !");
        return;
    }
    ...
    if (!player)
    {
        URHO3D_LOGERRORF("GameNetwork() - LoadEquipment ... No Player !");
        return;
    }

    player->LoadStuffOnly();
}
...
void GameNetwork::SendChangeEquipment(const StringHash& eventType, VariantMap& eventData)
{
    ...
    PushObjectCommand(UPDATEEQUIPMENT, &eventData);
}
...
void GameNetwork::Client_CommandRemoveObject(VariantMap& eventData)
{
    unsigned nodeid = eventData[Net_ObjectCommand::P_NODEID].GetUInt();
...
void GameNetwork::Client_ApplyObjectCommand(VariantMap& eventData)
{
    int cmd = eventData[Net_ObjectCommand::P_COMMAND].GetInt();
...
//        SetEnableObject(eventData[Net_ObjectCommand::P_NODEISENABLED].GetBool(), eventData[Net_ObjectCommand::P_NODEID].GetUInt());
    ...
    URHO3D_LOGINFOF("GameNetwork() - Client_ApplyObjectCommand : NET_OBJECTCOMMAND : cmd=(%s) ... OK !", netCommandNames[cmd]);
}
...
ObjectControlInfo* GameNetwork::AddSpawnControl(Node* node, Node* holder, bool enable)
{
    ...
    if (node && holder != node)
    {
        ...
        // 23/09/2021 : for foreign spawn, if a small gap between the stamps exists, we need to make up for
//        if (holderclientid != clientID_)
//        {
//            const unsigned char& receivedstamp = receivedSpawnStamps_[holderclientid];
//            if (receivedstamp - localstamp == 1)
//            {
//                URHO3D_LOGERRORF("GameNetwork() - AddSpawnControl : clientid=%d node=%s(%u) ... make up for localstamp=%u receivedstamp=%u !",
//                                    clientID_, node ? node->GetName().CString() : "null", nodeid, localstamp, receivedstamp);
//                localstamp++;
//            }
//        }
    ...
    oinfo->SetActive(true, serverMode_);
    ...
    URHO3D_LOGINFOF("GameNetwork() - AddSpawnControl : clientid=%d node=%s(%u) ... spawnid=%u(holderid=%u,spawnstamp=%u) ... %s oinfo=%u servernodeid=%u clientnodeid=%u holderptr=%u(nodeid=%u,clientid=%d) spawnstamp=%u !",
                    clientID_, node ? node->GetName().CString() : "null", nodeid, spawnid, GetSpawnHolder(spawnid), GetSpawnStamp(spawnid), newinfo ? "new":"", oinfo, oinfo->serverNodeID_, oinfo->clientNodeID_, holder, holderid, holderclientid, spawnstamp);
    return oinfo;
}
...
//    if (change)
//    {
//        URHO3D_LOGINFOF("GameNetwork() - LinkSpawnControl : object from clientid=%d spawnid=%u links serverNodeID_=%u clientNodeID_=%u changed=%s !",
//                        oinfo->clientId_, spawnid, oinfo->serverNodeID_, oinfo->clientNodeID_, change?"true":"false");
//    }
}
...
void GameNetwork::NetAddEntity(ObjectControlInfo& info, Node* holder, VariantMap* eventData, bool enable)
{
...
//            URHO3D_LOGINFOF("GameNetwork() - Server_SendGameStatus : Send Server Status=%s(%d) to All Clients !", gameStatusNames[status], status);
    }
}
...
void GameNetwork::Server_SendSeedTime(unsigned time)
{
    ...
    for (HashMap<Connection*, ClientInfo>::ConstIterator it = serverClientInfos_.Begin(); it != serverClientInfos_.End(); ++it)
    {
        ...
        if (clientInfo.connection_)
            clientInfo.connection_->SendRemoteEvent(NET_SERVERSEEDTIME, true, remoteEventData);
    }
}
    ...
    for (Vector<SharedPtr<Player > >::Iterator it = clientInfo.players_.Begin(); it != clientInfo.players_.End(); ++it)
    {
        ...
        if (!avatar)
        {
            ...
            if (avatar)
            {
                ...
                ObjectControlInfo& objectControlInfo = GetOrCreateServerObjectControl(nodeid, nodeid, clientInfo.clientID_);
                objectControlInfo.node_ = avatar;
...
void GameNetwork::Server_AddConnection(Connection* connection, bool clean)
{
    ...
    if (!serverClientInfos_.Contains(connection))
    {
        ...
        if (Server_CheckAccount(connectkey))
        {
            ...
            URHO3D_LOGINFOF("GameNetwork() - Server_AddConnection : connection=%u IP=%s ... OK !", connection, connection->ToString().CString());
...
void GameNetwork::Client_AddServerControllable(Node*& node, ObjectControlInfo& info)
{
    ...
    else
    {
        ...
        /// TODO : create a specific network event/handle for changing equipment
        int entityid = info.GetReceivedControl().states_.entityid_;
...
ObjectControlInfo& GameNetwork::GetOrCreateServerObjectControl(unsigned servernodeid, unsigned clientnodeid, int clientid)
{
    if (servernodeid)
    {
        for (Vector<ObjectControlInfo>::Iterator it = serverObjectControls_.Begin(); it != serverObjectControls_.End(); ++it)
    ...
    if (servernodeid && serverMode_)
    {
        ...
        else if (!GameContext::Get().rootScene_->IsPoolNodeReserved(servernodeid) && !GameContext::Get().rootScene_->IsNodeReserved(servernodeid))
            GameContext::Get().rootScene_->ReserveNodeID(servernodeid);
    }
    else if (clientnodeid && clientMode_)
    {
        if (!GameContext::Get().rootScene_->IsPoolNodeReserved(clientnodeid) && !GameContext::Get().rootScene_->IsNodeReserved(clientnodeid))
...
//                                servernodeid, clientnodeid, controlInfo.clientId_, &controlInfo);
    ...
    if (node || (!GameContext::Get().rootScene_->IsPoolNodeReserved(clientnodeid) && !GameContext::Get().rootScene_->IsNodeReserved(clientnodeid) && clientnodeid == servernodeid))
    {
        if (!node)
            GameContext::Get().rootScene_->ReserveNodeID(clientnodeid);
        newcontrol = true;
...
void GameNetwork::Client_SubscribeToPlayEvents()
{
    ...
    SubscribeToEvent(NET_SERVERSEEDTIME, URHO3D_HANDLER(GameNetwork, HandlePlayClient_MessagesFromServer));
//    SubscribeToEvent(NET_OBJECTCOMMAND, URHO3D_HANDLER(GameNetwork, HandlePlayClient_MessagesFromServer));
    SubscribeToEvent(GO_INVENTORYSLOTEQUIP, URHO3D_HANDLER(GameNetwork, HandlePlayClient_Messages));
    SubscribeToEvent(GO_INVENTORYLOAD, URHO3D_HANDLER(GameNetwork, HandlePlayClient_Messages));
...
void GameNetwork::Client_UnsubscribeToPlayEvents()
{
    ...
    UnsubscribeFromEvent(NET_OBJECTCOMMAND);
    UnsubscribeFromEvent(GO_INVENTORYSLOTEQUIP);
    UnsubscribeFromEvent(GO_INVENTORYLOAD);
...
GameStatus CheckReceivedGameStatus(GameStatus currentStatus, unsigned receivedStamp, GameStatus receivedStatus, unsigned char& lastReceivedStamp)
{
    ...
    if (sync && receivedStatus > MENUSTATE && currentStatus == MENUSTATE)
        return PLAYSTATE_INITIALIZING;

    if (currentStatus == PLAYSTATE_ENDGAME && receivedStatus == PLAYSTATE_STARTGAME)
        return PLAYSTATE_STARTGAME;

    if (currentStatus > MENUSTATE && currentStatus <= receivedStatus)
...
void GameNetwork::HandleServer_MessagesFromClient(StringHash eventType, VariantMap& eventData)
{
    ...
    {
        Connection* connection = static_cast<Connection*>(eventData[RemoteEventData::P_CONNECTION].GetPtr());
...
//            URHO3D_LOGINFOF("GameNetwork() - HandleClient_MessagesFromServer : NET_GAMESTATUSCHANGED clientID=%d currStamp=%u received=%u DESYNC !", clientID_, lastReceivedTimeStamp_, timeStamp);
        ...
        else if (serverGameStatus >= PLAYSTATE_SYNCHRONIZING && serverGameStatus <= PLAYSTATE_RUNNING)
        {
            if (gameStatus_ == PLAYSTATE_SYNCHRONIZING)
            {
                if (needSynchronization_)
                {
                    if (gameStatus_ != lastGameStatus_)
                        URHO3D_LOGINFOF("GameNetwork() - HandleClient_MessagesFromServer : NET_GAMESTATUSCHANGED clientID=%d ... ready ... serverstatus=%d status=%d", clientID_, serverGameStatus_, gameStatus_);

                    // All Clients Ready
                    if (serverGameStatus == PLAYSTATE_RUNNING)
                        lastGameStatus_ = gameStatus_ = PLAYSTATE_READY;
                }
                else
                {
                    // Launch Playing Directly
                    if (serverGameStatus >= PLAYSTATE_SYNCHRONIZING)
                        lastGameStatus_ = gameStatus_ = PLAYSTATE_READY;
                }
            }
...
void GameNetwork::HandlePlayClient_MessagesFromServer(StringHash eventType, VariantMap& eventData)
{
    if (!clientSideConnection_)
    {
        URHO3D_LOGWARNINGF("GameNetwork() - HandlePlayClient_MessagesFromServer : no connection to server !");
        return;
    }

    if (eventType == NET_SERVERSEEDTIME && !seedTime_)
    {
        seedTime_ = eventData[Net_ServerSeedTime::P_SEEDTIME].GetUInt();
        URHO3D_LOGINFOF("GameNetwork() - HandlePlayClient_MessagesFromServer : NET_SERVERSEEDTIME : seedtime=%u", seedTime_);
    }
}
...
void GameNetwork::HandlePlayClient_Messages(StringHash eventType, VariantMap& eventData)
{
    ...
    else if (eventType == GO_INVENTORYLOAD)
        PushObjectCommand(LOADINVENTORY, &eventData);

    else if (eventType == GO_DROPITEM)
        PushObjectCommand(REMOVEITEM, &eventData);
}
...
//        URHO3D_LOGINFOF("GameNetwork() - UpdateControl : node=%u ... ", node->GetID());
        ...
        {
            controller->ChangeAvatar(objectControl.states_.type_, objectControl.states_.entityid_);
        }
        ...
        // Update Physics
        bool physicOk = setposition ? GameHelpers::SetPhysicProperties(node, &objectControl.physics_, false, true) : false;
...
void GameNetwork::CleanObjectCommands()
{
    ...
    {
        if (serverClientInfos_.Size())
            oldestObjectCmdAck = serverClientInfos_.Begin()->first_->GetServerObjCmdAck();
        else
            oldestObjectCmdAck = Connection::GetServerObjCmd();

        if (serverClientInfos_.Size() > 1)
        {
            HashMap<Connection*, ClientInfo>::Iterator it = serverClientInfos_.Begin();
            it++;
            for ( ; it != serverClientInfos_.End(); ++it)
            {
                ...
                if (!IsNewStamp(connection->GetServerObjCmdAck(), oldestObjectCmdAck))
                    oldestObjectCmdAck = connection->GetServerObjCmdAck();
            }
        }
    }
    else
    ...
    for (Vector<ObjectCommand>::Iterator it = objectCommands_.Begin(); it != objectCommands_.End(); ++it)
    {
        ...
        {
//            URHO3D_LOGINFOF("GameNetwork() - CleanObjectCommands : clean from clientid=%d cmd=%s cmdstamp=%u oldestObjectCmdAck=%u !",
//                            cmd.clientId_, netCommandNames[cmd.cmd_[Net_ObjectCommand::P_COMMAND].GetInt()], cmd.stamp_, oldestObjectCmdAck);
            cmd.clientId_ = -1;
        }
    }
}
...
void GameNetwork::HandlePlayServer_ReceiveUpdate(StringHash eventType, VariantMap& eventData)
{
    ...
    // Receive the client netstatus
    {
        ...
        if (clientstatus >= PLAYSTATE_STARTGAME && clientstatus <= PLAYSTATE_RUNNING)
        {
...
//                            clientID_, nodeclientid, servernodeid, clientnodeid, oinfo ? oinfo->clientNodeID_:0, oinfo, spawnid, GetSpawnHolder(spawnid), GetSpawnStamp(spawnid));
            ...
            if (avatarnode && !objectControlInfo.node_ && sDumpObject_.IsEnable())
            {
                ...
                NetAddEntity(objectControlInfo);
...
//                if (!avatarnode)
        ...
        else if (type == OBJECTCOMMAND)
        {
            ...
            if (cmd.clientId_ != clientID_ && cmd.clientId_ >= 0 && IsNewStamp(cmd.stamp_, clientSideConnection_->GetServerObjCmdAck()))
            {
#ifdef ACTIVE_NETWORK_DEBUGCLIENT_OBJECTCOMMANDS
                URHO3D_LOGINFOF("GameNetwork() - HandlePlayClient_ReceiveServerUpdate : apply ObjectCommand from clientid=%d cmdstamp=%u serverObjCmdAck=%u ...",
    ...
    if (gameStatus_ >= PLAYSTATE_READY && gameStatus_ <= PLAYSTATE_RUNNING)
    {
        ...
        if (needSynchronization_ && !allClientsSynchronized_)
        {
            ...
            for (HashMap<Connection*, ClientInfo>::Iterator it = serverClientInfos_.Begin(); it != serverClientInfos_.End(); ++it)
            {
                ...
                if (clientInfo.gameStatus_ > MENUSTATE)
                {
                    if (clientInfo.gameStatus_ < PLAYSTATE_SYNCHRONIZING)
                        Server_SendGameStatus(PLAYSTATE_SYNCHRONIZING, clientInfo.connection_);
                    else
                        synchronizedClients++;
                }
            }
...
//                URHO3D_LOGINFOF("GameNetwork() - HandlePlayServer_NetworkUpdate : All Clients are Synchronized !");
        ...
        if (!allClientsRunning_ && (!needSynchronization_ || (needSynchronization_ && allClientsSynchronized_)))
        {
            ...
            for (HashMap<Connection*, ClientInfo>::Iterator it = serverClientInfos_.Begin(); it != serverClientInfos_.End(); ++it)
            {
                ...
                if (clientInfo.gameStatus_ < PLAYSTATE_RUNNING)
    ...
    if (objectCommands_.Size())
    {
        ...
        for (HashMap<Connection*, ClientInfo>::Iterator it = serverClientInfos_.Begin(); it != serverClientInfos_.End(); ++it)
        {
            ...
            for (Vector<ObjectCommand>::Iterator jt = objectCommands_.Begin(); jt != objectCommands_.End(); ++jt)
            {
                ...
                if (clientid != cmd.clientId_ && IsNewStamp(cmd.stamp_, connection->GetServerObjCmdAck()))
                {
#ifdef ACTIVE_NETWORK_DEBUGSERVER_OBJECTCOMMANDS
                    URHO3D_LOGINFOF("GameNetwork() - HandlePlayServer_NetworkUpdate : send to clientid=%d cmdstamp=%u serverObjCmd=%u serverObjCmdAck=%u !",
                                    clientid, cmd.stamp_, Connection::GetServerObjCmd(), connection->GetServerObjCmdAck());
#endif
            ...
            if (numcmdsended > 0)
            {
//                URHO3D_LOGINFOF("GameNetwork() - HandlePlayServer_NetworkUpdate : to clientid=%d numcmdsended=%u/%u serverObjCmd=%u serverObjCmdAck=%u",
//                                clientid, numcmdsended, objectCommands_.Size(), Connection::GetServerObjCmd(), connection->GetServerObjCmdAck());

#ifdef ACTIVE_NETWORK_LOGSTATS
                clientinfo.tmpLogStats_.stats_[ObjCmdSent] += numcmdsended;
...
//    URHO3D_LOGINFOF("GameNetwork() - HandlePlayClient_NetworkUpdate ... gamestatus=%s(%d)", gameStatusNames[gameStatus_], gameStatus_);
    ...
    if (gameStatus_ == PLAYSTATE_CLIENT_LOADING_SERVEROBJECTS)
    {
        SetGameStatus(PLAYSTATE_SYNCHRONIZING);
        return;
    }
...
void GameNetwork::DumpClientInfos() const
{
    ...
    {
        URHO3D_LOGINFOF("GameNetwork() - DumpClientInfos : connectionToServer=%u clientid=%d gamestatus=%s(%u)", clientSideConnection_.Get(), clientID_, gameStatusNames[gameStatus_], gameStatus_);
